...
module tb_ldi;
    ...
    initial begin
        // CASE 3: ldi R7, 0x65
        // Reverted to positive constant: (Op=0, Ra=7, Rb=0, C=0x65) -> Hex: 03800065
        DUT.ram_inst.memory[ 9'h000 ] = 32'h03800065; 
        
        // CASE 4: ldi R0, 0x72(R2)
        // (Op=0, Ra=0, Rb=2, C=0x72) -> Hex: 00100072
    ...
    initial begin
        ...
        // FORCE the value directly into the hardware register for Case 4
        force DUT.R2.Q = 32'h0000_0057;
    ...
    always @(posedge clk) begin
        if (clr) Present_state <= Default;
            ...
            case (Present_state)
                ...
                T5_1: Present_state <= T0_2; // Transitions smoothly to Case 4 after T5
