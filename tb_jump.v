...
module tb_jump;
    ...
    // --------------------------------------------------------
    initial begin
        // Simulation 1: jr R12 at Address 0x10
        // (Op=20, Ra=12) -> Hex: A6000000
        DUT.ram_inst.memory[ 9'h010 ] = 32'hA6000000; 

        // Simulation 2: jal R4 at Address 0x20
        // Phase 2 Datapath Hack: To manually route PC->R12 and R4->PC without the Phase 3 Control Unit, 
        // we embed both registers into the instruction fields. (Op=19, Ra=12, Rb=4) -> Hex: 9E200000
        DUT.ram_inst.memory[ 9'h020 ] = 32'h9E200000; 
    end

    // --------------------------------------------------------
    // DELAYED REGISTER PRELOAD & EXPLICIT PC INJECTION
    // --------------------------------------------------------
    initial begin
        // Wait safely for the 25ns reset phase to drop
        #35; 
        
        // Simulation 1 Preloads
        force DUT.PC.Q = 32'h0000_0010;  // Start fetch at 0x10
        force DUT.R12.Q = 32'h0000_00FF; // Jump target for jr
        #10; 
        release DUT.PC.Q; 
        release DUT.R12.Q;
        
        // Fast forward past Simulation 1's execution (t=120ns)
        #75; 
        
        // Simulation 2 Preloads
        force DUT.PC.Q = 32'h0000_0020;  // Force fetch to 0x20
        force DUT.R4.Q = 32'h0000_0088;  // Jump target for jal
        force DUT.R12.Q = 32'h0000_0000; // Clear R12 to explicitly watch it capture the return address
        #10;  
        release DUT.PC.Q; 
        release DUT.R4.Q;
        release DUT.R12.Q;
    ...
    // --------------------------------------------------------
    initial begin $dumpfile("tb_jump.vcd"); $dumpvars; end
    ...
    parameter Default = 5'd0, 
              ...
              T0_2=5'd5, T1_2=5'd6, T2_2=5'd7, T3_2=5'd8, T4_2=5'd9;
    ...
    always @(posedge clk) begin
        if (clr) Present_state <= Default;
            ...
            T3_2: Present_state <= T4_2; T4_2: Present_state <= T4_2; 
        endcase
    end
    ...
    always @(Present_state) begin
        ...
        case (Present_state)
            ...
            T3_1: begin Gra = 1; Rout = 1; PCin = 1; end // Dump R12 strictly into PC
            ...
            T3_2: begin PCout = 1; Gra = 1; Rin = 1; end // Store incremented PC into R12
            T4_2: begin Grb = 1; Rout = 1; PCin = 1; end // Dump R4 into PC to execute the jump
        endcase
    end
endmodule