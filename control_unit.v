...
// Control Unit: Generates all control signals to operate the processor Datapath.
// It accepts op-codes from the IR, Datapath condition signals (CON_FF), and external inputs (Reset, Stop).
module control_unit (
    // System inputs
    ...
    // Datapath routing and memory interface signals
    output reg Yin, Zin, PCout, IncPC, MARin, MDRin, MDRout,
    output reg Read, Write, Clear,
    
    // ALU operation selectors
    output reg ADD_op, SUB_op, MUL_op, DIV_op, SHR_op, SHRA_op, SHL_op, 
    output reg ROR_op, ROL_op, AND_op, OR_op, NEG_op, NOT_op,
    
    // Special register enables and execution state indicators
    output reg HIin, LOin, CONin, PCin, IRin, OutPortin, Cout, Zlowout, Zhighout,
    output reg HIout, LOout, InPortout, Run
);

    // FSM State Definitions: Maps execution steps (T0, T1, etc.) to unique state values.
    parameter [5:0]
        reset_state = 6'd0,
        fetch0      = 6'd1,
        fetch1      = 6'd2,
    ...
    // FSM State Transition Block: Determines the next state on every positive clock edge.
    always @(posedge Clock, posedge Reset) begin
        if (Reset == 1'b1) begin
            present_state <= reset_state; // Force CPU into initial reset state
        end
        else if (Stop == 1'b1) begin
            present_state <= halt_state;  // Halt control stepping process
        end
        else begin
            case (present_state)
                reset_state: present_state <= fetch0;
                
                // Instruction Fetch Sequence (T0, T1, T2)
                fetch0:      present_state <= fetch1;
                fetch1:      present_state <= fetch2;
                fetch2:      present_state <= decode_state; 
                
                // Instruction Decoding: Branches to specific execution paths based on IR[31:27] opcode
                decode_state: begin
                    case (IR[31:27])
                        ...
                        5'b10101: present_state <= br3;   // branch (brzr, brnz, brpl, brmi)
                ...
                // Memory Subsystem Execution States (ld, ldi, st)
                ...
                // Multiplication and Division Execution States
                ...
                // Input/Output and HI/LO Move Operations
    ...
    // FSM Output Logic Block: Asserts required datapath signals for the active state.
    always @(present_state or CON_FF) begin
        // Initialize all control signals to zero to prevent latching
        ...
        case (present_state)
            // Fetch Sequence: Independent of instruction op-codes
            ...
            // Immediate ALU Operations (utilizes the sign-extended Cout)
